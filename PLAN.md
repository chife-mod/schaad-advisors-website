...
## Адаптивный дизайн hero · кольца услуг

Hero сейчас спроектирован под десктоп (≥1024). Кольца сервисов — полоса 4×клетки шириной max 1440px, центрированная, с bleed −140px на белую Leistungen. На больших экранах работает идеально, но на ужении окна возникает проблема:

**Проблема.** Высота клетки = `strip_w / 4`, а bleed зафиксирован 140px. На 1440 это 39% клетки (метка label остаётся в синей зоне). На 768 это 73% — метка уезжает на белый фон Leistungen и становится невидимой (белый текст на белом).
...
Решение откладывается на следующий round дизайн-итераций — после approve десктопа клиентом.
