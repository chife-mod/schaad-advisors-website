...
Hero сейчас спроектирован под десктоп (≥1024). Все позиции — абсолютные пиксели от Figma 1440×800. Делается responsive после approve десктопа клиентом.

### Стратегия брейкпоинтов

Три брейкпоинта, каждый меняет layout-стратегию hero, не косметику:

| Брейкпоинт | Диапазон | Layout |
|---|---|---|
| **Desktop** | ≥1024 | Текущий: лого 328×205 top-left, nav центр-вверх, лид-блок и right-блок по углам, портрет справа, кольца bleed на белое |
| **Tablet** | 736–1023 | Лого ужимается, nav прижимается к лого + кнопка справа, портрет уменьшается, right-блок переносится **под кнопки** в одну колонку с лид-текстом, кольца на всю ширину |
| **Mobile** | <736 | Одна колонка, портрет на всю ширину или скрыт, кольца → 2×2 grid или чипсы (см. ниже) |

### Tablet (736–1023) — конкретика

По эскизу клиента в Figma на ширине 736:

1. **Лого** — `width: 228px` (было 328px), `height: auto`, ratio сохранён.
2. **Nav** — снимаем `left: calc(var(--gutter) + 546px)`, ставим `flex-direction: row`, прижимаем к лого через flex-gap. `width: auto`.
3. **Kontakt-кнопка** — остаётся в правом верхнем углу.
4. **Gutter** — `--rail-pad` уменьшается с 80 до 24–32px (через `clamp()` или media query).
5. **Лид-блок** — single column, full-width минус gutter. Headline `clamp()` на меньший размер.
6. **Right-блок** — критичная перестройка: `position: static`, переезжает в DOM-flow внутрь `.hero__lead` ПОД кнопки. Внутри signature → строка с `claim | loc` (flex row, 2 колонки).
7. **Portrait** — `width: clamp(280px, 40vw, 468px)`, прижимается к правому краю через `right: var(--rail-pad)`.
8. **Hero высота** — `min-height: 720px` вместо фикс 720, потому что вертикальный stack контента может расти.
9. **Кольца** — растягиваются на всю ширину viewport (current `min(100vw, 1440px)` уже это даёт). Лейблы продолжают работать через тот же `:has()`-механизм.

### Mobile (<736)

1. **Лого** — ещё меньше, ~180×113.
2. **Nav** — гамбургер-меню (новый компонент) или скрытая всплывающая навигация.
3. **Лид-блок** — full-width, шрифты ещё меньше через `clamp()`.
4. **Portrait** — варианта 2: либо во всю ширину сверху как горизонтальный кроп (head & shoulders), либо скрыт совсем, оставив hero textually-only.
5. **Кольца** — отдельная проблема, см. ниже.

### Кольца сервисов · responsive

**Проблема.** На десктопе кольца — полоса 4×клетки шириной max 1440px, центрирована, bleed −140px на белую Leistungen. Высота клетки = `strip_w / 4`. Bleed зафиксирован 140px. На 1440 это 39% клетки (метка label остаётся в синей зоне). На 768 это 73% — метка уезжает на белый фон Leistungen и становится невидимой.

**4 варианта решения, по нарастанию радикальности:**

1. **Пропорциональный bleed.** Связать bleed с размером клетки, например `bottom: calc(-1 * var(--strip-w) / 16)` (≈ 25% клетки на любой ширине). Метка остаётся в той же относительной зоне. Минимальная правка, но кольца на мобильном всё равно растягиваются на полную ширину — выглядит несоразмерно.

2. **Брейкпоинт ≥768.** До 768 — текущий ряд из 4 клеток. Ниже — сетка 2×2 без bleed (кольца помещаются в hero как декоративный грид). Сохраняет визуальную идентичность, не ломает иерархию.

3. **Кольца только на desktop, чипсы на tablet/mobile.** ≥1024 — как сейчас. <1024 — простой ряд из 4 пилюль с названиями (как теги в CTA-секции). Кольца становятся «фишкой главного экрана», на мобильном уступают место читаемости. Самое швейцарское решение.

4. **Hero без колец.** Кольца выносятся в самостоятельную секцию между hero и Leistungen — никогда не bleed, всегда контейнер 1440×360 на тёмном. Решает адаптив радикально, но теряется fishка «кольца обнимают портрет».

**Рекомендация (гибрид 1 + 3):**
- Desktop (≥1024): кольца как сейчас, но bleed становится `% от клетки`, не фиксированный 140.
- Tablet/Mobile (<1024): кольца отключаются. Под hero — компактный ряд из 4 чипсов, кликабельных. Никакого bleed-конфликта, никакой пропадающей типографики.

### Когда делаем

Не сейчас. Адаптив — последний этап перед production:

1. Approve десктопа клиентом (Verena → Jakob).
2. Approve остальных трёх страниц (Über mich, Lesezeichen, Kontakt) — десктоп.
3. Только после этого — единый sprint адаптива, потому что каждое изменение десктопа сейчас потребовало бы синхронизации со всеми брейкпоинтами.
